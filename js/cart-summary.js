...
$(document).ready(function(){
	$('.cart_quantity_up').off('click').on('click', function(e){
		e.preventDefault();
		upQuantity( $(this).attr('id').replace('cart_quantity_up_', '')  ,  parseInt( $("#quantity_"+$(this).attr('id').replace('cart_quantity_up_', '')).val() )+1);
		$('#' + $(this).attr('id').replace('_up_', '_down_')).removeClass('disabled');
	});
	$('.cart_quantity_down').off('click').on('click', function(e){
		e.preventDefault();
		var rowid = $(this).attr('id').replace('cart_quantity_down_', '') ;
		upQuantity( rowid  ,  parseInt( $("#quantity_"+rowid).val() )-1);
...
function upQuantity(id, qty)
{
	...
	$.ajax({
		type: 'POST',
		headers: { "cache-control": "no-cache" },
		url: baseUri + 'cart/updateQtyForCart/?rowid=' + productId + "&qty="+qty +"&ajax=true",
		async: true,
		cache: false,
		dataType: 'json',
		success: function(jsonData)
...
function updateCartSummaryNew(json){
	...
	for (i=0;i<json.products.length;i++)
		product_list[json.products[i].id] = json.products[i];

	for (i in product_list) {
		var rowid = product_list[i].rowid ;
		$("#quantity_"+rowid).val(product_list[i].quantity);
		$("#total_product_price_"+rowid).html( product_list[i].sub_total +" QAR" );

	}
	$("#total_product").html(json.total +" QAR");
