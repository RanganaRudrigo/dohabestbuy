/**
 * Created by Gowtham on 12/19/14.
 */
...
$(function(){
    ...
    $('body').find('.country-list').each(function(){
        ...
        for(i=0;country.length > i ;i++ ){
            op += "<option value='"+ country[i].nicename +"' ";
            op  += country[i].nicename == country_id ? "selected":"";
            if(country[i].id == country_id) ph = country[i].phonecode;
            op += " >"+ country[i].name +"</option>";}

        $(this).html(op);
        if($(this).hasClass('licence')) {
            ops = "<option value='1000' ";
            ops  += 1000 == country_id ? "selected":"";
            ops += " > INTERNATIONAL </option>";
            $(this).append(ops);
            console.log('lic');
        }
